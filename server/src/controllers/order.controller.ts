...
export const createCheckoutSession = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const products = await Promise.all(
      items.map((item: any) => prisma.product.findUnique({ where: { id: item.productId }, include: { shop: true } }))
    );
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { buyerId: req.user!.id, items: JSON.stringify(items) },
    });
...
export const stripeWebhook = async (req: any, res: Response): Promise<void> => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const order = await prisma.order.create({
      data: {
        buyerId, shopId: shop.id, pickupPointId,
    ...
    await sendEmail({ to: order.buyer.email, subject: 'Commande confirmée — EcoMarket', html: `<h2>Commande confirmée ✅</h2><p>Votre commande #${order.id.slice(0, 8)} a été reçue.</p>` });
  }
