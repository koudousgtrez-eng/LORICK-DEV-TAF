...
export const getShops = async (req: Request, res: Response): Promise<void> => {
  try {
    const shops = await prisma.shop.findMany({
      ...
      include: { products: { where: { isPublished: true }, select: { id: true, name: true, price: true, photos: true } } },
    });
    res.json(shops);
...
export const getShop = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const shop = await prisma.shop.findUnique({
      where: { id },
      include: { products: { where: { isPublished: true }, orderBy: { createdAt: 'desc' } }, owner: { select: { firstName: true, lastName: true } } },
    });
...
export const createShop = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const shop = await prisma.shop.create({
      data: { name, description, address: address || '', latitude: parseFloat(latitude), longitude: parseFloat(longitude), ownerId: req.user!.id, status: 'APPROVED' as any },
    });
